...
function finalizeForm() {
	...
	var pageCollectionId = getElementValue("pageCollectionId").toLowerCase();
	var found=false;
	for (i=0;i<destinationSelect.options.length;i++) {
		if (destinationSelect.options[i].text.toLowerCase()==pageCollectionId) {
			found=true;
			break;
		}
	}
	if (!found) {
		for (i=0;i<select.options.length;i++) {
				if (select.options[i].text.toLowerCase()==pageCollectionId) {
