...
ADD COLUMN `numberEntries` BIT(1) NULL AFTER `useLocationDesc`;

DELIMITER ;
USE `food`;

ALTER TABLE `food`.`feature` 
ADD COLUMN `reverseOrder` BIT(1) NULL AFTER `numberEntries`;
...
CREATE PROCEDURE addFeature(name varchar(100), headline varchar(300), subhead varchar(300), author varchar(200), datePosted datetime, introContent text, closingContent text, locationCriteria varchar(500), locationTemplate text, useLocationDesc bit, numberEntries bit, reverseOrder bit, tenantid int, userid int)
BEGIN
...
CREATE PROCEDURE updateFeature(_id int, name varchar(100), headline varchar(300), subhead varchar(300), author varchar(200), datePosted datetime, introContent text, closingContent text, locationCriteria varchar(500), locationTemplate text, useLocationDesc bit, numberEntries bit, reverseOrder bit, _tenantid int, _userid int)
BEGIN
